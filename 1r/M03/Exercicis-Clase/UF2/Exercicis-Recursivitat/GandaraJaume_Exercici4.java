...
public class GandaraJaume_Exercici4 {
    /**
     * Mètode main que crida al mètode menu()
     * @param args
     */
    public static void main(String[] args) {
        int mida = inputIntMin("Quants números vols multiplicar?\n > ","ERROR: Tipatge incorrecte",2,"ERROR: Introdueix un valor més gran que 0");
        float[] vector = new float[mida];

        omplirVector(vector);

        System.out.println("El resultat és " + multiplicaVector(vector, mida-2, vector[vector.length-1]));
    }
    ...
    /**
     * Mètode per multiplicar tots els números d'un vector
     * @param vector de tipus float[]
     * @param mida de tipus int
     * @return resultat de tipus float[] que és la multiplicació de tots els números del vector
     */
    private static float multiplicaVector(float[] vector, int mida, float resultat){
        if(mida == -1){
            return resultat;
        }else {
            resultat *= vector[mida];
            return resultat = multiplicaVector(vector,mida-1,resultat);
        }
    }
